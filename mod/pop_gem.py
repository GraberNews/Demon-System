...
class PopGemScreen(BaseChatScreen):

    def on_kv_post(self, base_widget):
        self.setup_keyboard_handler()
        self.setup_input_colors()
        self.bottom_menu = BottomMenu(
            items=get_menu_items("gem"),
            on_select=self.handle_menu
        )
        self.history = []
        self.api_chat_id = "gem"
        # Назва провайдера береться з manage_api.py (блок ЧАТ: GEM)
        self.ids.chat_logo.text = APIManager.get_provider_name("gem")
    ...
    def handle_menu(self, action):
        ...
        actions = {
            ...
            "files":   lambda: print("GEM files"),
            "db":      lambda: print("GEM DB"),
        }
    ...
    def update_text(self):
        app = MDApp.get_running_app()
        try:
            self.ids.top_bar.title = app.lang.get("gem_title")
