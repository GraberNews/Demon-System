...
# Кл. Початок. NumericProperty більше не потрібен тут — він визначений у BaseChatScreen
# Кл. Кінець
...
<PopAIScreen>:
    ...
    FloatLayout:
        ...
        # ── Головна колонка — ScrollView + TopAppBar ──
        MDBoxLayout:
            id: root_box
        ...
        # size_hint_y: None + height: self.minimum_height — фіксований розмір по контенту.
        MDBoxLayout:
            id: input_area
            orientation: "vertical"
            size_hint: 1, None
            ...
            ChatInputCard:
                ...
                MDTextField:
                    ...
                    # Кл. Початок. Виправлено on_focus:
                    # — при фокусі: ховаємо hint (поле активне)
                    # — при втраті фокусу: _refresh_hint() повертає hint
                    #   тільки якщо поле порожнє (коректна поведінка)
                    on_focus:
        ...
        # ── FAB меню — поверх контенту, під TopAppBar ──
        MDIconButton:
            id: btn_top_menu
            icon: "dots-vertical"
            icon_size: "16sp"
...
class PopAIScreen(BaseChatScreen):
    ...
    def on_kv_post(self, base_widget):
        ...
        self.history = []
        self.api_chat_id = "ai"
        # Назва провайдера береться з manage_api.py (блок ЧАТ: AI)
        self.ids.chat_logo.text = APIManager.get_provider_name("ai")
    ...
    def handle_menu(self, action):
        ...
        actions = {
            ...
            "test":    lambda: app.switch_screen("test"),
            "files":   lambda: print("AI files"),
            "db":      lambda: print("AI DB"),
