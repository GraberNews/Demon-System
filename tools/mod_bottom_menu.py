...
class BottomMenu(ModalView):
    ...
    def __init__(self, items=None, on_select=None, **kwargs):
        ...
        # Кл. Початок. Зберігаємо стабільний референс на bound method
        # щоб unbind гарантовано знаходив той самий об'єкт
        self._lang_handler = self._on_language_change
        # Кл. Кінець
    ...
    # Кл. Початок. Перейменовано on_language_change → _on_language_change
    # (приватний метод, референс зберігається в self._lang_handler)
    ...
    def open(self, *args):
        ...
        # Кл. Початок. Використовуємо self._lang_handler замість self.on_language_change
        # unbind через стабільний референс гарантує відсутність дублів
        app = MDApp.get_running_app()
        app.lang.unbind(current=self._lang_handler)
        app.lang.bind(current=self._lang_handler)
        # Кл. Кінець
    ...
    def _finish_dismiss(self, *args):
        # Кл. Початок. Відписуємось через стабільний референс
        try:
            app = MDApp.get_running_app()
            if app:
                app.lang.unbind(current=self._lang_handler)
